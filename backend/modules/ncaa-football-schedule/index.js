// backend/modules/ncaa-football-schedule/index.js
...
class NCAAFootballScheduleModule extends BaseModule {
  constructor() {
    super({
      id: 'ncaa_football_schedule',
      name: 'NCAA Football Schedule',
      league: 'NCAA',
      sport: 'football',
      dataType: 'schedule',
  ...
  transformScheduleData(eventsData, teamId) {
    ...
    return eventsData.map(event => {
      ...
      return {
        ...
        result: result, // e.g., "W 45-38" or "L 21-24"
        ...
        // Tournament info (bowl games, playoffs)
        ...
        notes: event.notes || event.media?.game_note
      };
    });
  }
  ...
  async fetchTeamSchedule(team, options = {}) {
    try {
      ...
      // Check if team has football sport ID
      const sportConfig = team.ncaaSportsConfig?.football;
      if (!sportConfig?.sportId) {
        throw new Error(`Team ${team.teamId} missing football sport ID. Run auto-populate first.`);
      }
      ...
      logger.debug(`📅 Fetching football schedule from unified endpoint: ${scheduleUrl}`);
      ...
      const response = await retryWithBackoff(async () => {
        return await axios.get(scheduleUrl, {
          ...
          headers: {
            ...
            'Pragma': 'no-cache',
            'Referer': `${baseUrl}/sports/football/schedule`
          }
        });
      }, `${team.teamName} football schedule fetch`, 3, 5000);

      // Transform the schedule data
      const games = this.transformScheduleData(response.data, team.teamId);
      logger.debug(`Found ${games.length} football games for ${team.teamName}`);
      ...
      logger.error(`Error fetching football schedule for ${team.teamName || team.teamId}:`, error.message);
      ...
      if (error.code === 'ECONNREFUSED') {
        ...
        throw new Error(`Schedule not found. Sport ID ${team.ncaaSportsConfig?.football?.sportId} may be incorrect.`);
...
module.exports = NCAAFootballScheduleModule;
