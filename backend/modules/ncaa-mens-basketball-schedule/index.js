// backend/modules/ncaa-mens-basketball-schedule/index.js
...
class NCAAMensBasketballScheduleModule extends BaseModule {
  constructor() {
    super({
      id: 'ncaa_mensBasketball_schedule',
      name: 'NCAA Mens Basketball Schedule',
      league: 'NCAA',
  ...
  transformScheduleData(eventsData, teamId) {
    ...
    return eventsData.map(event => {
      ...
      return {
        ...
        result: result, // e.g., "W 85-78" or "L 65-72"
  ...
  async fetchTeamSchedule(team, options = {}) {
    try {
      ...
      // Check if team has mens basketball sport ID
      const sportConfig = team.ncaaSportsConfig?.mensBasketball;
      if (!sportConfig?.sportId) {
        throw new Error(`Team ${team.teamId} missing mens basketball sport ID. Run auto-populate first.`);
      }
      ...
      logger.debug(`🏀 Fetching mens basketball schedule from unified endpoint: ${scheduleUrl}`);
      ...
      const response = await retryWithBackoff(async () => {
        return await axios.get(scheduleUrl, {
          ...
          headers: {
            ...
            'Pragma': 'no-cache',
            'Referer': `${baseUrl}/sports/mens-basketball/schedule`
          }
        });
      }, `${team.teamName} mens basketball schedule fetch`, 3, 5000);

      // Transform the schedule data
      const games = this.transformScheduleData(response.data, team.teamId);
      logger.debug(`Found ${games.length} mens basketball games for ${team.teamName}`);
      ...
      logger.error(`Error fetching mens basketball schedule for ${team.teamName || team.teamId}:`, error.message);
      ...
      if (error.code === 'ECONNREFUSED') {
        ...
        throw new Error(`Schedule not found. Sport ID ${team.ncaaSportsConfig?.mensBasketball?.sportId} may be incorrect.`);
...
module.exports = NCAAMensBasketballScheduleModule;
