// backend/modules/ncaa-womens-basketball-schedule/index.js
...
class NCAAWomensBasketballScheduleModule extends BaseModule {
  constructor() {
    super({
      id: 'ncaa_womensBasketball_schedule',
      name: 'NCAA Womens Basketball Schedule',
      league: 'NCAA',
  ...
  transformScheduleData(eventsData, teamId) {
    ...
    return eventsData.map(event => {
      ...
      return {
        ...
        result: result, // e.g., "W 78-65" or "L 60-70"
        ...
        // Tournament info (conference tournaments, March Madness, etc.)
  ...
  async fetchTeamSchedule(team, options = {}) {
    try {
      ...
      // Check if team has womens basketball sport ID
      const sportConfig = team.ncaaSportsConfig?.womensBasketball;
      if (!sportConfig?.sportId) {
        throw new Error(`Team ${team.teamId} missing womens basketball sport ID. Run auto-populate first.`);
      }
      ...
      logger.debug(`🏀 Fetching womens basketball schedule from unified endpoint: ${scheduleUrl}`);
      ...
      const response = await retryWithBackoff(async () => {
        return await axios.get(scheduleUrl, {
          ...
          headers: {
            ...
            'Pragma': 'no-cache',
            'Referer': `${baseUrl}/sports/womens-basketball/schedule`
          }
        });
      }, `${team.teamName} womens basketball schedule fetch`, 3, 5000);

      // Transform the schedule data
      const games = this.transformScheduleData(response.data, team.teamId);
      logger.debug(`Found ${games.length} womens basketball games for ${team.teamName}`);
      ...
      logger.error(`Error fetching womens basketball schedule for ${team.teamName || team.teamId}:`, error.message);
      ...
      if (error.code === 'ECONNREFUSED') {
        ...
        throw new Error(`Schedule not found. Sport ID ${team.ncaaSportsConfig?.womensBasketball?.sportId} may be incorrect.`);
...
module.exports = NCAAWomensBasketballScheduleModule;
